...
static void glfw_mouse_press(GLFWwindow* window, int button, int action, int modifier)
{
  ...
  if (action == GLFW_PRESS)
  {
	  ...
	  { 
		  ...
		  if(!found)
			  found = rndr->Picking(x2, y2, &z);
		  else
			  rndr->Picking(x2, y2, &z);
		  if (prev_z != z) 
			  j = i;
	  ...
	  else 
	  {
		  ...
		  if (j != savedIndx) {
			  scn->data_list[savedIndx].set_colors(Eigen::RowVector3d(255.0 / 255.0, 228.0 / 255.0, 58.0 / 255.0));
		  }
		  Eigen::Vector3f amt = (scn->data().MakeTrans() * Eigen::Vector4f(0, 0, 0, 1)).head(3);
...
static void glfw_mouse_scroll(GLFWwindow* window, double x, double y)
{
	...
	if (y < 0) {
		if (!rndr->GetScene()->is_picked) {
			rndr->GetScene()->TranslateInSystem(rndr->GetScene()->MakeTrans(), Eigen::Vector3f(0, 0, -1), true);
		}
		else {
